...
async def update_live_location(
    ...
    """
    🔴 CRITICAL FIX: Heartbeat endpoint for Live Tracking (Bug #2 FIX from Audit).
    
    Updates location AND broadcasts new coordinates to contacts every 30 seconds.
    Handles timezone-aware/naive datetime comparison safely.
    """
    alert = db.query(AlertLog).filter(AlertLog.id == alert_id).first()
    if not alert:
        raise HTTPException(status_code=404, detail="Alert session not found")
    ...
    # ✅ FIXED: Properly handle timezone-aware vs timezone-naive datetimes
    should_broadcast = True
    if alert.last_location_broadcast is not None:
        try:
            last_broadcast = alert.last_location_broadcast
            
            # If database returns naive datetime, make it aware (assume UTC)
            if last_broadcast.tzinfo is None:
                last_broadcast = last_broadcast.replace(tzinfo=timezone.utc)
            
            # Now safe to compare
            time_since_last = now - last_broadcast
            should_broadcast = time_since_last >= timedelta(seconds=30)
        except Exception as e:
            logger.error(f"Broadcast timing calculation failed: {e}")
            # Fallback: always broadcast if calculation fails
            should_broadcast = True
    
    # Always update the database with current location
    alert.latitude = latitude
    alert.longitude = longitude
    
    # If 30+ seconds have passed, broadcast the new location to all contacts
    if should_broadcast:
        try:
            # Fetch updated user and contacts
            user = db.query(User).filter(User.id == alert.user_id).first()
            contacts = (
                db.query(EmergencyContact)
                .filter(EmergencyContact.user_id == alert.user_id)
            ...
            if contacts and user:
                ...
                # Send updated location to all contacts with media if available
                send_alerts_to_contacts(contacts, updated_message, media_url=alert.media_url)
                
                # Update the broadcast timestamp to now (timezone-aware)
                alert.last_location_broadcast = now
                logger.info(f"✅ Live location broadcast to {len(contacts)} contacts")
        except Exception as e:
            logger.error(f"❌ Live broadcast failed: {e}")
...
async def upload_incident_photo(
    ...
    """
    🔴 CRITICAL FIX: Receive live photo and location.
    Hardened ImgBB upload + passes media_url to all messaging services.
    """
    print(f"\n[INCIDENT] PROCESSING ALERT FOR USER: {user_id}")
    ...
    # 6. Broadcasts to all channels
    results = []
    
    # EMAIL BROADCAST
    try:
        print("📧 TRIGGERING EMAIL BROADCAST...")
        email_results = send_email_alerts_to_contacts(
            contacts=contacts, 
        ...
        results.extend(email_results)
        print("✅ EMAIL BROADCAST COMPLETE!")
    except Exception as e: 
        logger.error(f"Email broadcast failed: {e}")
        print(f"❌ EMAIL BROADCAST FAILED: {e}")

    # TWILIO BROADCAST (SMS/WhatsApp) - ✅ NOW WITH media_url!
    try:
        print("📱 TRIGGERING TWILIO BROADCAST...")
        # ✅ FIXED: NOW PASSING media_url TO TWILIO!
        twilio_results = send_alerts_to_contacts(contacts, sos_message, media_url=media_url)
        results.extend(twilio_results)
        print("✅ TWILIO BROADCAST COMPLETE!")
    except Exception as e: 
        logger.error(f"Twilio broadcast failed: {e}")
        print(f"❌ TWILIO BROADCAST FAILED: {e}")

    # TELEGRAM BROADCAST (Free & Automated)
    try:
    ...
    alert_log = AlertLog(
        ...
        media_url=media_url,
        # ✅ FIXED: Set initial broadcast timestamp
        last_location_broadcast=datetime.now(timezone.utc)
    )
    db.add(alert_log)
    db.commit()
    db.refresh(alert_log)

    print(f"\n✅ INCIDENT ALERT LOGGED: {alert_log.id}\n")
...
async def trigger_alert(data: AlertTrigger, db: Session = Depends(get_db)):
    """
    Direct SOS trigger — for voice/button activation without photo.
    Broadcasts to all messaging channels.
    """
    print(f"\n[TRIGGER] DIRECT SOS FOR USER: {data.user_id}")
    ...
    if not contacts:
        raise HTTPException(status_code=400, detail="No emergency contacts configured for this user")

    # 3. Get medical info and reverse geocode
    med = db.query(MedicalInfo).filter(MedicalInfo.user_id == data.user_id).first()
    address = await reverse_geocode(data.latitude, data.longitude) if data.latitude and data.longitude else None

    # 4. Generate Rich SOS Message
    profile_url = f"{settings.BASE_URL}/scan/{data.user_id}"
    ...
    # TWILIO BROADCAST (SMS/WhatsApp) - No photo in trigger, so no media_url
    try:
        print("📱 TRIGGERING TWILIO BROADCAST...")
        twilio_results = send_alerts_to_contacts(contacts, sos_message)
        results.extend(twilio_results)
    ...
    print(f"\n✅ TRIGGER ALERT LOGGED: {alert_log.id}\n")
...
@router.get("/history")
def get_alert_history(user_id: str, db: Session = Depends(get_db)):
    """Get alert history for a user — last 20 alerts."""
    alerts = (
        db.query(AlertLog)
