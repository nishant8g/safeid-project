      ...
      <div style={{ width: '100%', padding: '24px', display: 'flex', justifyContent: 'center', zIndex: 10 }}>
        ...
        >
          ...
          <Link 
            ...
            onMouseOver={(e) => { e.currentTarget.style.color = 'var(--accent-emerald)'; e.currentTarget.style.background = 'rgba(16, 185, 129, 0.1)'; }}
            onMouseOut={(e) => { e.currentTarget.style.color = 'var(--text-secondary)'; e.currentTarget.style.background = 'rgba(255, 255, 255, 0.05)'; }}
          >
            <ArrowLeft size={14} /> Back to Home
          </Link>

          <div style={{ width: '80px', height: '80px', background: 'rgba(16, 185, 129, 0.1)', borderRadius: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 32px', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
            <ShieldCheck size={40} color="var(--accent-emerald)" strokeWidth={2.5} />
          </div>
          ...
          <p style={{ marginTop: '40px', fontSize: '0.9rem', color: 'var(--text-muted)' }}>
            Already have an account? <span onClick={() => navigate('/login')} style={{ color: 'var(--accent-cyan)', cursor: 'pointer', fontWeight: '700' }}>Sign In</span>
          </p>
